...
| 7. BigQuery リソース | 7-1 ログシンク宛先データセット（log_sinks 由来）／7-2 アセットインベントリ（asset_inventory・iam_policy・v_iam_policy＝IAM 監査・基盤標準） |
| 8. 監視・予算 | 8-1 アラート定義／8-2 通知先／8-3 予算アラート |
| 9. ネットワーク | 9-1 Shared VPC サブネット／9-2 VPC-SC 境界 |
| 10. タグ・ラベル | 10-1 組織タグ定義（`tag_definitions`）／10-2 プロジェクト別 ラベル(`app`/`env`/`owner`)・適用タグ(`org_tags`) |
| 11. Google グループ・IAM | 管理用 Google グループと付与される組織レベル IAM ロール（集約／フルモード・`enable_group_iam` の適用状態を反映） |
| 12. 費用注意事項 | コストが増加しやすい設定（Data Access 監査ログ集約・VPC フローログ・シンク先 BigQuery/GCS・Monitoring 取り込み等）の注意事項と抑制策 |

### 判定ロジックの要点

- **実施状況（1. 構築項目一覧）**: 対応する SSoT の定義が存在すれば `完了`、無ければ `—`。
  例）`resources` にフォルダ行が無ければ「管理フォルダ構成 → —」。
- **ラベル（10-2）**: GCP プロジェクトの `labels`。`app`（= `resource_name`）は常に付与、`env` / `owner` は SSoT に値がある場合のみ付与（空欄は付与なし）。`make generate` が生成する `terraform.tfvars` の挙動と一致します。
- **組織タグ（10-1）**: 組織レベルのタグ。`common.tfvars` の `enable_tags` 有効時に組織へ作成され、各プロジェクトの `org_tags`（`key/value` 形式）に基づき紐付けられます。
- **BigQuery リソース（7）**: 7-1 は `log_sinks` の BigQuery 宛先データセット（パーティション表・保持日数＝テーブル既定の有効期限）。7-2 は基盤標準の `asset_inventory`（CAI の IAM_POLICY フィード宛先・IAM 監査台帳）で、SSoT 非依存の固定構成として記載。
