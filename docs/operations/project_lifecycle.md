...
### SSoT (Excel) からのプロジェクト削除時の挙動について

Excelからプロジェクトの行を削除し `make generate` を実行すると、当該プロジェクト内の `terraform.tfvars` が自動的に削除され、以降のデプロイ対象から外れます（孤立状態による安全なパージ）。
この際、現場のコードを保護するため GCP 上の実リソースやローカルのディレクトリ自体は削除されません。リソースを完全に解体する場合は、Excelから行を消す前に `terraform.tfvars` の `deletion_protection = false` を設定し、手動で `terraform apply -destroy -target=module.project` を実行して解体を完了させてから、Excelの行を削除してください。
