...
### 4.1 Excel から削除したプロジェクトのディレクトリが残り続ける

#### 症状: `make generate` 後も `terraform/4_projects/<name>/` ディレクトリが残っている

- **原因**: これは意図的な動作です。`make generate` は `terraform.tfvars` を削除してデプロイ対象から除外しますが、現場が独自に追加したかもしれない `.tf` ファイルを誤って削除しないよう、ディレクトリ本体は保持されます。
- **対策**:
  1. GCP 上のリソースが解体済みであることを確認します（未解体の場合は先に `terraform apply -destroy -target=module.project` を実行）。
  1. 以下のコマンドで孤立ディレクトリを対話形式で一覧・削除します。
     ```bash
     make prune
     ```
  1. `PRUNE` と入力して確定すると、SSoT に存在しない全ディレクトリが削除されます。

> **注意**: `terraform.tfvars` がまだ残っているディレクトリ（`make generate` をまだ実行していない場合など）は `make prune` 実行時に警告付きで表示されます。GCP リソースが存在する可能性があるため、必ずリソースの解体を先に行ってください。
