...
変更が既存の機能に影響を与えていないか、また規約に沿っているかを確認するために、以下のコマンドを実行します。

1.  **ユニットテストの実行**: 各モジュールのディレクトリにある `.tftest.hcl` に基づき、`terraform test` を実行します。
    ```bash
    make test
    ```
2.  **静的解析とフォーマット**: コードの自動フォーマット、`tflint` によるエラーチェックを実行します。
    ```bash
    make lint
    ```
