世界一のクラウドエンジニアとして、ご要望のパラメータ一覧をエンタープライズ水準の「データ定義書（Data Dictionary）」としてMarkdown形式で書き起こしました。

このドキュメントは、そのまま `docs/reference/data_dictionary.md` としてリポジトリに配置し、引き継ぎ資料や顧客向けのマニュアルとしてご活用いただけます。
