...
SSoT（Excel）からプロジェクト行を削除して `make generate` を実行すると、`terraform.tfvars` が自動削除され、以降のデプロイ対象から外れます。しかし、カスタム `.tf` ファイルを誤って削除しないよう、**ディレクトリ本体は残ります**。

GCP 上のリソースを解体した後、以下のコマンドで孤立ディレクトリを対話形式で一括削除できます。

```bash
make prune
```

実行すると SSoT に存在しないプロジェクトディレクトリが一覧表示されます。確認後 `PRUNE` と入力することで削除が実行されます。なお、`terraform.tfvars` が残っているディレクトリは GCP リソースがまだ存在する可能性があるため、必ず事前に `terraform destroy` で解体してから実行してください。
