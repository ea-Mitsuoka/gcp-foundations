...
- `roles/orgpolicy.policyAdmin`: 組織ポリシーの管理
- `roles/compute.xpnAdmin`: Shared VPC の管理
- `roles/accesscontextmanager.policyAdmin`: VPC Service Controls の管理
...
terraform init -backend-config="$(git rev-parse --show-toplevel)/terraform/common.tfbackend" -reconfigure
terraform plan -var-file="$(git rev-parse --show-toplevel)/terraform/common.tfvars" -var-file="terraform.tfvars"
terraform apply -var-file="$(git rev-parse --show-toplevel)/terraform/common.tfvars" -var-file="terraform.tfvars"
```
