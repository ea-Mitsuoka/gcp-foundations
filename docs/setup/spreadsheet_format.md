...
| **existing_project_id** | 文字列 | 既存プロジェクト採用(adopt)モード用。既存の project_id を指定すると、命名規則 `<prefix>-<app_name>` に非準拠でも採用（`terraform import` 前提）。新規作成時は空欄。※`folder`の場合は無視。 | `existing-project-1234` |
