...
| **monitoring** | ブール値 | 監視対象とするか。※`folder`の場合は無視されます。 | `TRUE`, `FALSE` |
| **logging** | ブール値 | ログ集約対象とするか。※`folder`の場合は無視されます。 | `TRUE`, `FALSE` |
...
| 項目 | 定義場所 | 主な用途 |
| :--- | :--- | :--- |
| **デフォルトリージョン** | `common.tfvars` (`gcp_region`) | ログ集約 (BQ/GCS)、監視、Cloud Functions 等の組織共通リソースの配置場所。 |
...
2.  **コストとレイテンシ**: 共通基盤（ログ送信先等）とサブネットが地理的に離れている場合、リージョン間データ転送コストやネットワークレイテンシが発生します。
3.  **入力の徹底**: Excel の `region` が空欄の場合、不完全な Terraform コードが生成されエラーの原因となります。
...
#### 8.3 ネットワーク設計の検証 (Network Validation)
...
`resources` シートの `logging` および `monitoring` 列の挙動には以下の特性があります。

#### ログ収集フラグ (`logging`)

- **挙動**: `FALSE` にしても **ログ転送は停止しません**。
- **理由**: セキュリティ統制のため、監査ログ等の収集は「組織シンク」レベルで一括設定されています。
- **用途**: 主にプロジェクトに付与されるラベル (`logging: true/false`) や、管理台帳上の目印として使用します。ログ費用を削減したい場合は、中央の `log_sinks` シートで除外フィルタを設定してください。

#### モニタリングフラグ (`monitoring`)

- **挙動**: `FALSE` にすると **監視対象から除外されます**。
- **詳細**: 監視プロジェクトから対象プロジェクトへのメトリクス読み取り権限（IAM）の付与がスキップされます。
- **結果**: Cloud Monitoring のダッシュボードやアラート通知にデータが現れなくなります。検証用の使い捨てプロジェクトなどでアラートノイズを減らしたい場合に `FALSE` を設定します。
