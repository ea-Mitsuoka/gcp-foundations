...
| **billing_account** | 文字列 | プロジェクトの課金アカウント指定（任意）。**空欄**＝グローバル `billing_account_id` にリンク（採用(adopt)プロジェクトで空欄の場合は既存リンクを尊重し `manual` 扱い）。**`manual`**＝Terraform は課金リンクを管理しない（手動運用/既存リンク前提。予算も作らない）。**`<課金アカウントID>`**＝指定アカウントにリンク。※課金リンクは作成時のみ設定（`ignore_changes`）で、既存プロジェクトの課金は付け替えない。※`folder`の場合は無視。 | ``, `manual`, `012345-6789AB-CDEF01` |
