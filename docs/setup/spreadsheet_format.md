...
| **resource_name** | 文字列 | フォルダ名、またはアプリ名。**プロジェクトはそのまま表示名になるため 4〜30 文字**（GCP プロジェクト表示名の制約）。 | `production`, `prd-app-01` |
| **environment** | 文字列 | プロジェクトの環境。`prod`/`stag`/`dev` のいずれか、または**空欄（任意）**。指定時のみ `env` ラベルが付与される（空欄＝ラベルなし）。名前からの推定はしない。**`shared_vpc` を使う行のみ必須**（接続先ホスト判定用）。 | `prod` |
| **existing_project_id** | 文字列 | 既存プロジェクト採用(adopt)モード用。既存の project_id を指定すると、命名規則 `<prefix>-<app_name>` に非準拠でも採用（`terraform import` 前提）。新規作成時は空欄。※`folder`の場合は無視。 | `` |
| **owner** | 文字列 | リソースの所有者を示す識別子。**任意**（空欄なら `owner` ラベルは付与されない）。指定時は `^[a-z0-9_-]{1,63}$` 形式（`@`や`.`は不可）。 | `infra-team` |
| **budget_amount** | 数値 | プロジェクトの月額予算。0より大きい場合、予算アラートが自動作成されます。 | `1000` |
| **budget_alert_emails** | 文字列 | 予算超過時の追加の通知先（複数指定はカンマ区切り）。 | `finance@example.com` |
...
| **billing_account** | 文字列 | プロジェクトの課金アカウント指定（任意）。**空欄**＝グローバル `billing_account_id` にリンク（採用(adopt)プロジェクトで空欄の場合は既存リンクを尊重し `manual` 扱い）。**`manual`**＝Terraform は課金リンクを管理しない（手動運用/既存リンク前提。予算も作らない）。**`<課金アカウントID>`**＝指定アカウントにリンク。※課金リンクは作成時のみ設定（`ignore_changes`）で、既存プロジェクトの課金は付け替えない。※`manual` のプロジェクトが未リンクの場合、`make deploy` は apply 前のチェックで停止する（手動リンク手順は [プロジェクトのライフサイクル管理](../operations/project_lifecycle.md) の「課金アカウントのリンク」を参照）。※`folder`の場合は無視。 | ``, `manual`, `012345-6789AB-CDEF01` |
...
#### 💡 フォルダ指定時の注意: 有効な列と無視される列

`resources` シートはフォルダとプロジェクトを同じシートで階層的に定義できますが、`resource_type = folder` の場合に**有効な列**と\*\*無視される列（プロジェクト専用）\*\*が明確に分かれています。

**フォルダ作成時に有効な列:**

| 列名 | 説明 |
| :--- | :--- |
| `resource_type` | `folder` を指定 |
| `parent_name` | 親リソース（`organization_id` または親フォルダ名） |
| `resource_name` | 作成するフォルダ名 |
| `org_tags` | フォルダレベルのタグ付け |

**フォルダ作成時に無視される列（プロジェクト専用）:**

| 列名 | 無視される理由 |
| :--- | :--- |
| `owner` | GCP プロジェクトのラベル（`owner` キー）として付与されるため |
| `budget_amount` / `budget_alert_emails` | 予算アラートはプロジェクト単位で設定されるため |
| `shared_vpc` | Shared VPC への参加はプロジェクト単位であるため |
| `vpc_sc` | VPC-SC の境界への参加はプロジェクト単位であるため |
| `central_monitoring` / `central_logging` | 監視・ログ集約の対象設定はプロジェクト単位であるため |

これらの列にフォルダ行で値が入力されていてもエラーにはなりません。`generate_resources.py` が自動的に読み飛ばします。テストケース **TC-018**（フォルダへの Shared VPC 設定無視確認）がこの挙動を保証しています。

> **ベストプラクティス**: フォルダ行を入力する際は、**A・B・C 列（resource_type / parent_name / resource_name）だけを記入し、残りの右側の列は空欄のままにしておく**のが最も明確な書き方です。
...
> ⚠️ **SSoT を経由せず手動追加したプロジェクトに注意**: コンソールや `gcloud` で手動作成し、この Excel に追記していないプロジェクトは、**中央監視の対象になりません**（監視スコープへの登録は本基盤のモジュール経由でプロジェクトを作成したときにのみ行われる Push 型のため）。一方で、**ログ集約は組織レベルのシンクで一括収集しているため、手動追加プロジェクトでも自動的に対象になります**。この非対称性の詳細と対処は [プロジェクトのライフサイクル管理](../operations/project_lifecycle.md) の「SSoT を経由せず手動追加したプロジェクトの『中央監視』と『ログ集約』」を参照してください。

#### 自動付与ラベル

`central_monitoring` と `central_logging` の値は、GCP プロジェクトに以下のラベルとして自動付与されます。

| ラベルキー | 元となる列 | 付与される値の例 |
| :--- | :--- | :--- |
| `monitoring` | `central_monitoring` | `true`, `false` |
| `logging` | `central_logging` | `true`, `false` |

これらは OPA ポリシー (`require_labels.rego`) が要求する必須ラベル（`env`, `owner`, `app`）とは別に自動生成されるものです。GCP コンソールやコスト管理ツールでの検索・フィルタリングに活用できます。
