...
上記の「prd-app-01」の行を定義して `make generate` を実行すると、以下のようなファイルが自動生成されます。

```plaintext
terraform/
├── 3_folders/
│   └── auto_folders.tf      # "production" フォルダの定義が追記される
└── 4_projects/
    └── prd-app-01/          # 新しくディレクトリが作成される
        ├── main.tf          # 標準テンプレートからコピー
        ├── terraform.tfvars # Excelの値（APIs, Shared VPC設定等）が注入される
        └── ...
```
