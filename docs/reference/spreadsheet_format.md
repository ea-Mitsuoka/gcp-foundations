...
この基盤では、`gcp_foundations.xlsx` を Single Source of Truth (SSOT) とし、このExcelファイルに定義を行うことで、GCPのフォルダ、プロジェクト、ネットワーク（サブネット）、セキュリティ境界（VPC-SC）が自動生成される仕組みを採用しています。
...
| **shared_vpc** | 文字列 | 使用する共有VPCサブネット名（`shared_vpc_subnets`シートで定義したもの） | `prd-subnet-01` |
| **vpc_sc** | 文字列 | 所属させるVPC-SC境界名（`vpc_sc_perimeters`シートで定義したもの） | `default_perimeter` |
...
| resource_type | parent_name | resource_name | shared_vpc | vpc_sc | billing_linked |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **folder** | `organization_id` | `shared` | | | FALSE |
| **folder** | `shared` | `production` | | | FALSE |
| **project** | `production` | `prd-app-01` | `prd-subnet-01` | `default_perimeter` | TRUE |

### 💡 動作のポイント

1.  **名前による紐付け**: `resources` シートの `shared_vpc` や `vpc_sc` 列には、他のシートで定義した「名前」を記述します。これにより、プロジェクトが正しいサブネットや境界に自動的に紐付けられます。
2.  **型変換**: スクリプトが内部的に Boolean や List 型に適切に変換して Terraform に渡します。
3.  **自動生成**: `make generate` を実行すると、これらの定義から `auto_folders.tf`, `auto_subnets.tf`, `auto_vpc_sc.tf` が各レイヤーに生成されます。
