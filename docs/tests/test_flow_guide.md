...
- **コード生成のクリーンアップ**: `make generate` を実行すると、前回生成された `auto_*.tf` ファイルは自動的に削除されます。したがって、ローカルの `.tf` ファイルの手動削除は不要です。
- **GCPリソースのクリーンアップ**: 前のテストで作成したリソースが次のテストの邪魔になる場合（例：同名のプロジェクトを作成しようとする場合）、`make destroy` を実行して更地にするか、Excel側でリソース名をユニークにする必要があります。
- **状態の引き継ぎ**: 特に指定がない限り、各テストは「前のテストが完了した状態（GCP上にリソースがある状態）」から開始することを想定していますが、バリデーションエラー系（生成のみ）のテストはGCP側に影響を与えません。
...
| **TC-000a** | **TC-000b** | `common.tfvars` が生成されていることを確認。権限不足の別アカウントに切り替えて検証。 |
...
| 現在のテスト | 次のテスト | 遷移作業・準備 |
| :--- | :--- | :--- |
| **TC-000** | **TC-001** | `gcp-foundations.xlsx` を新規作成（または `TC001_to_003_Basic.xlsx` をコピー）。 |
| **TC-001** | **TC-002** | `resources` シートに行を追記（フォルダと子プロジェクト）。前のプロジェクトを残したままでも可。 |
| **TC-002** | **TC-003** | `resources` シートの既存行に `budget_amount` を追記。 |
| **TC-003** | **TC-004** | `TC004_to_009_Validation_Err.xlsx` にファイルを差し替える（`make generate` で生成が止まることを確認するフェーズへ）。 |
...
| 現在のテスト | 次のテスト | 遷移作業・準備 |
| :--- | :--- | :--- |
| **TC-009** | **TC-010** | `TC010_016_019_Network_Success.xlsx` に差し替え。`common.tfvars` で `enable_shared_vpc = true` に設定。 |
| **TC-010** | **TC-011** | `TC011_012_014_015_018_Network_Err.xlsx` に差し替え（重複CIDRの確認）。 |
...
| **TC-015** | **TC-016** | ネットワークエラーを解消し、Excelを `prod` / `dev` 両方のサブネット定義に更新。 |
...
| 現在のテスト | 次のテスト | 遷移作業・準備 |
| :--- | :--- | :--- |
| **TC-019** | **TC-020** | `TC020_to_022_026_Governance_Success.xlsx` に差し替え。`common.tfvars` で `enable_vpc_sc = true` に設定。 |
...
| **TC-022** | **TC-023** | `TC023_024_025_027_028_Governance_Err.xlsx` に差し替え（定義外タグ値）。 |
...
| 現在のテスト | 次のテスト | 遷移作業・準備 |
| :--- | :--- | :--- |
| **TC-029** | **TC-030** | Excelで既存フォルダの `resource_name` を書き換える。 |
...
| 現在のテスト | 次のテスト | 遷移作業・準備 |
| :--- | :--- | :--- |
| **TC-039** | **TC-040** | `TC040_to_041_Logging_Success.xlsx` に差し替え。ハイフン入りの `destination_parent` を入力。 |
