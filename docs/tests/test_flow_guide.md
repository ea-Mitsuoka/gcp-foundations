...
| 現在のテスト | 次のテスト | 遷移作業・準備 |
| :--- | :--- | :--- |
| **TC-009** | **TC-010** | ⚠️ **【最終決定版：デッドロックからの完全回復手順】**<br>異常系テスト後は、**①L4プロジェクトの残骸**と**②L1管理プロジェクトのロック**という2つの要因でデッドロックが発生します。以下の手順でこれらを解消し、環境を完全に更地に戻してください。<br><br>1. **正常なExcelに戻す:**<br> `(ロック解除を適用するため、実在するプロジェクトが定義された状態に戻します)`<br> `cp tests/fixtures/TC001_to_003_Basic.xlsx gcp-foundations.xlsx`<br>2. **変数を再生成:**<br> `make generate`<br>3. **L4の残骸を物理削除:**<br> `rm -rf terraform/4_projects/<失敗したプロジェクト名>`<br> `rm -f deploy_state.json`<br>4. **L1管理プロジェクトのロックを強制解除:**<br> `cd terraform/1_core/base/logsink && terraform apply -var-file="../../../common.tfvars" -auto-approve`<br> `cd ../monitoring && terraform apply -var-file="../../../common.tfvars" -auto-approve`<br> `cd ../vpc-host && terraform apply -var-file="../../../common.tfvars" -auto-approve`<br> `cd ../../../../`<br>5. **L4プロジェクト群のロックを解除:**<br> `common.tfvars` で `allow_resource_destruction=true` を確認。<br> `make deploy`<br>6. **完全解体の実行:**<br> `make destroy ALL`<br>7. **次期テストの準備:**<br> `TC010_016_019_Network_Success.xlsx` を `gcp-foundations.xlsx` にコピー後、`common.tfvars` で `enable_shared_vpc = true` に設定。 |
