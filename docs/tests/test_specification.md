...
| **TC-000e** | Bootstrap による admin/network フォルダ作成の確認 | 1. `make setup` (または `cd terraform/0_bootstrap && terraform apply`) を実行。<br>2. `cd terraform/0_bootstrap && terraform output` を実行。 | `admin_folder_id` と `network_folder_id` が `folders/XXXXXXXXXX` 形式で出力され、GCP Console 上で組織直下に `admin` フォルダと `network` フォルダが存在すること。 | | | | |
...
| **TC-013b** | 矛盾フラグの検知 | 1. `common.tfvars` で `enable_shared_vpc=true` かつ `enable_vpc_host_projects=false` に設定。<br>2. `make generate` を実行。 | `generate_resources.py` がエラー（`enable_shared_vpc = true には enable_vpc_host_projects = true が必要です`）を出力し、生成が停止すること。 | | | | |
...
| **TC-045** | vpc-host の L3 非依存デプロイ確認 | 1. クリーンな環境から `make deploy` を 1 回だけ実行（`3_folders` 後の vpc-host 再実行は行わない）。<br>2. 完了後 `cat .deploy_state` を確認。 | vpc-host が L3 (folders) 完了前に正常に apply され、全レイヤーがエラーなく 1 パスで完了すること。`.deploy_state` で `terraform/1_core/base/vpc-host` が `terraform/3_folders` より前の行に記録されていること。 | | | | |
...
| **TC-044** | 管理プロジェクトの正しいフォルダ配置確認 | 1. `make deploy` 後に以下を実行。<br>`gcloud projects describe <PREFIX>-logsink --format="value(parent.id)"`<br>`gcloud projects describe <PREFIX>-monitoring --format="value(parent.id)"`<br>`gcloud projects describe <PREFIX>-vpc-prod --format="value(parent.id)"`<br>`gcloud projects describe <PREFIX>-tfstate-xxxx --format="value(parent.id,parent.type)"` | logsink・monitoring が `admin` フォルダID 配下に、vpc-prod・vpc-dev が `network` フォルダID 配下に配置されること。tfstate プロジェクトのみ `parent.type` が `organization` (フォルダではなく組織直下) であること。 | | | | |
...
| テストID | テスト項目 | テスト手順 | 期待される結果 | 結果 (OK/NG) | 確認者 | 実施日 | 備考 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **TC-040** | BQ ID サニタイズ | 1. `log_sinks` の `destination_parent` にハイフン入りの名前を指定。<br>2. `make generate`<br>3. `cat terraform/1_core/services/logsink/sinks/auto_global_vars.tf` (確認) | ハイフンがアンダースコア `_` に自動置換され、BigQueryの命名規則に従っていること。 | OK | 光岡 | 2026/05/14 | |
...
| **TC-043** | ドキュメント空欄の回避 | 1. `alert_documentation` 列を意図的に空にする。<br>2. `make generate`<br>3. 生成されたコードの `documentation` 属性を確認。 | デフォルトの "No documentation provided." に自動補完されていること。 | OK | 光岡 | 2026/05/14 | validate機能としても確認ずみ |
