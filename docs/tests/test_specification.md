...
| **TC-042** | アラート定義の重複検知 | 1. `alert_definitions` に全く同じ `alert_name` を2行作成。<br>2. `make generate` | Python側で重複エラーとして検知され、Terraformのパニックを回避できること。 | OK | 光岡 | 2026/05/14 | |
| **TC-043** | ドキュメント空欄の回避 | 1. `alert_documentation` 列を意図的に空にする。<br>2. `make generate`<br>3. 生成されたコードの `documentation` 属性を確認。 | デフォルトの "No documentation provided." に自動補完されていること。 | OK | 光岡 | 2026/05/14 | |
