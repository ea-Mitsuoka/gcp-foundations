...
| テストID | テスト項目 | テスト手順 | 期待される結果 | 結果 (OK/NG) | 確認者 | 実施日 | 備考 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **TC-040** | BQ ハイフン禁止バリデーション | 1. `log_sinks` の `destination_parent` にハイフン入りの名前（例: `error-logs`）を指定。<br>2. `make generate` | `generate_resources.py` の `ResourceValidator` が「BigQueryのデータセット名にハイフン(-)は使用できません」というエラーを出力し、生成が停止すること。（以前の「Terraform側で `_` に自動置換する」動作は廃止済み） | | | | 仕様変更: 自動サニタイズ → バリデーションエラー |
...
| **TC-043** | alert_documentation 必須バリデーション | 1. `alert_definitions` の `alert_documentation` 列を意図的に空にする。<br>2. `make generate` | `generate_resources.py` が「`alert_documentation`（説明）は必須項目です。空欄は許可されません。」というエラーを出力し、生成が停止すること。（以前の「Terraform側で "No documentation provided." に自動補完する」動作は廃止済み） | | | | 仕様変更: 自動補完 → バリデーションエラー |
| **TC-046** | CAI BigQuery View (`v_iam_policy`) の存在確認 | 1. `make deploy` 完了後に以下を実行。<br>`bq show --format=prettyjson <PREFIX>-logsink:asset_inventory.v_iam_policy`<br>または GCP Console で `logsink` プロジェクト → BigQuery → `asset_inventory` データセットを確認。 | `v_iam_policy` ビューが存在し、`event_time`, `asset_type`, `resource_name`, `policy_bindings`, `is_deleted` の各カラムが定義されていること。ビューのクエリが `iam_policy` 物理テーブルを参照していること。 | | | | |
...
| **TC-048** | `make test-mode` のON/OFF切り替え | 1. `make test-mode` を実行（OFF→ON）。<br>2. `common.tfvars` の `project_id_prefix` と `.test_mode_env` の内容を確認。<br>3. 再度 `make test-mode` を実行（ON→OFF）。<br>4. `common.tfvars` と `.test_mode_env` の状態を確認。 | 1. `project_id_prefix` に2文字のランダムサフィックス（例: `-a3`）が付与され、`.test_mode_env` に `TEST_SUFFIX` が記録されること。<br>3. `project_id_prefix` が元の値に正確に戻り、`.test_mode_env` が削除されること。手動でプレフィックスを変更していた場合も、記録したサフィックス分だけ正確に除去されること。 | | | | |
