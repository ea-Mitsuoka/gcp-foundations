...
| **TC-015** | ホストビットが立っているCIDR | 1. `shared_vpc_subnets` にホストビットが設定されたCIDR（例: `10.0.1.5/24`）を入力。<br>2. `make generate` | Pythonの `ResourceValidator` が Invalid CIDR format エラー（has host bits set）を出力すること。 | | | | |
