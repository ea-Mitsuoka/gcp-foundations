...
`make generate` によって生成される各プロジェクトには、以下のラベルが付与されます。**いずれも任意**で、OPA による必須ラベル強制は**撤廃**しています（プロジェクトの識別は `project_id` で行う方針）。`env`/`owner` は空欄ならラベル自体が付与されません。

| ラベルキー | 必須 | 値の内容 | 値の生成元 |
| :--- | :---: | :--- | :--- |
| `env` | 任意 | `prod` / `stag` / `dev` | `resources` シートの `environment` 列（空欄ならラベルなし。section 2 参照） |
| `owner` | 任意 | 所有者を示す識別子 | Excel `resources` シートの `owner` 列（`^[a-z0-9_-]{1,63}$` 形式。空欄ならラベルなし） |
| `app` | 任意 | アプリ名 | Excel `resources` シートの `resource_name` 列（常に値あり） |
...
**現在、必須ラベルは撤廃**されています（`policies/require_labels.rego` の `required_labels` は空集合）。そのため `deny` は発火せず、ラベル欠如でブロックされることはありません（プロジェクトの識別は `project_id` で行う方針）。OPA の仕組み自体は残しているため、将来ラベルを必須化したくなった場合は `required_labels` に追加するだけで有効化できます。削除アクションは元々対象外です。
